...
const NotificationScreen: React.FC<{
  ...
  const [failedAvatarLoads, setFailedAvatarLoads] = useState<
    Record<string, boolean>
  >({});
  const [failedThumbLoads, setFailedThumbLoads] = useState<
    Record<string, boolean>
  >({});
  ...
  const fetchNotifications = useCallback(async () => {
    try {
      ...
      const rawArray = Array.isArray(responseData?.data)
        ? responseData.data
        : [];
      ...
      const mapped: Notification[] = rawArray.map((n: any) => {
        ...
        if (n.organizerName || n.organizerAvatarUrl) {
          ...
          avatarSource =
            n.authorAvatarUrl ?? n.fromUserAvatar ?? n.organizerAvatarUrl;
        } else if (n.fromUserName || n.fromUserAvatar) {
          nameSource = n.fromUserName ?? n.authorName ?? n.organizerName;
          avatarSource =
            n.fromUserAvatar ?? n.authorAvatarUrl ?? n.organizerAvatarUrl;
        }
        ...
        const organizerAvatarUrl = toAbsolute(
          API_BASE_URL,
          n.organizerAvatarUrl
        );
  ...
  const handleNotificationPress = useCallback(
    (notification: Notification) => {
      ...
      if (
        (notification.category === "event" || notification.type === "event") &&
      ...
      if (
        (notification.category === "post" ||
          ["post", "like", "comment"].includes(notification.type || "")) &&
  ...
  const getNotificationIcon = (type?: string, category?: string) => {
    ...
    switch (notificationType.toLowerCase()) {
      case "like":
        return { name: "heart" as const, color: "#FF5722" };
      case "comment":
        return { name: "chatbubble-outline" as const, color: "#F9662D" };
      case "follow":
        return { name: "person-add-outline" as const, color: "#FF5722" };
      case "event":
      ...
      >
        ...
        <View style={styles.avatarContainer}>
          {hasAvatar ? (
            ...
            <View
              style={[styles.defaultAvatar, { backgroundColor: icon.color }]}
            >
              <Text style={styles.defaultAvatarText}>
                {(item.organizerName || item.fromUserName || item.title)
                  .charAt(0)
                  .toUpperCase()}
              </Text>
            </View>
          ...
          <View style={styles.postThumbnail}>
            {hasThumbnail ? (
              <Image
                ...
                onError={() => {
                  console.warn("Post image failed to load", {
                    id: item.id,
                    thumbUri,
                  });
              ...
              <View style={styles.placeholderImage}>
                <Ionicons
                  name={
                    item.category === "event" ? "calendar" : "image-outline"
                  }
                  size={20}
                  color="#ccc"
                />
  ...
  return (
    <View style={{ flex: 1 }}>
      <SafeAreaView style={styles.container}>
        ...
        <LinearGradient colors={["#FF5722", "#F9662D"]} style={styles.header}>

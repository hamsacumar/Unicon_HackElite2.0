...
export const verifyEmail = async (email: string, code: string) => {
  try {
    ...
    if (token) {
      ...
      console.log(
        "Token saved successfully, stored token exists:",
        !!storedToken
      );
    } else {
...
export const resendCode = async (
  ...
  try {
    ...
    const errorMessage =
      responseData.message || "Failed to process your request";
    ...
    if (error instanceof Error) {
      if (
        ...
        error.message.includes("2 minutes")
      ) {
        throw new Error(
          "Please wait 2 minutes before requesting a new verification code."
        );
...
export const login = async (usernameOrEmail: string, password: string) => {
  try {
    ...
    if (!response.ok) {
      ...
      const errorMessage =
        loginResponse.message || response.statusText || "Login failed";
    ...
    return {
      ...
      email:
        userInfo.Email || (isEmail(usernameOrEmail) ? usernameOrEmail : ""),
